...
mocha.describe('Erasure decoding test suite', function () {
  ...
  mocha.it('Decode smaller than k * stripe size (1 data + 1 partiy)', function (done) {
    ...
    const decoder = ecstream.decode(ostream, 1000, [dataStream, null], [parityStream], 512);
  ...
  mocha.it('Decode multiple of stripe size (1 data + 1 partiy)', function (done) {
    ...
    const decoder = ecstream.decode(ostream, 1024, [null, dataStream], [parityStream], 512);
    assert.strictEqual(decoder.k, 2);
    assert.strictEqual(decoder.m, 1);
  ...
  mocha.it('Back pressure', function (done) {
    ...
    const decoder = ecstream.decode(ostream, 8192, [null, dataStream], [parityStream], 1024);
    const encodeMethodBackup = decoder.encode;
    decoder.encode = (dataBuffer, parityBuffer, dispatcher) => setTimeout(
      () => encodeMethodBackup.apply(decoder, [dataBuffer, parityBuffer, dispatcher]),
      300,
    );
  });

  mocha.it('Large stripe size', function (done) {
    const stripeSize = ecstream.safeStripeSize(2, 1024 * 1024 * 8);
    ...
    ecstream.decode(ostream, 2 * stripeSize, [null, dataStream], [parityStream], stripeSize);
  });
});
